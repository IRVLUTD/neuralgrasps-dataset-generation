...
def eval_exp(s):
  ...
  # return const, coeff
  # return const, coeff, str(var)
  # str(var) == 'd2' or 'd10' or something like that (0-indexed)
  # we want the dof index associated with that joint!
  return const, coeff, int(str(var)[-1])
...
def convert(input_filename, data_dir, output_dir):
  ...
  scale = 1e3 # keep it 1 for fetch gripper
  ...
  def convert_link(g_link):
    ...
    scale_str = (' '.join(['{:.8f}']*3)).format(1, 1, 1)
  ...
  for chain in g_hand.findall('chain'):
    ...
    for joint in chain.findall('joint'):
      ...
      if joint_idx == 1:
        ...
        tx, ty, tz = np.fromstring(init_T.find('translation').text, sep=' ') / scale
        rx = ry = rz = 0
        try:
      ...
      try:
        ...
        joint_type = 'revolute'
        dh_alpha, ax, dof_idx = eval_exp(dh_alpha)
      ...
      joint_idx += 1
      # dof_idx += 1 # error! this assumes all dofs are linearly described.
      # but fails for the case when multiple joints are coupled together with same dof
