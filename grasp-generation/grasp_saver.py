...
class GraspSaver:
    ...
    def __call__(self, body_name, body_grasps, gripper_name, total_plans, current):
        ...
        Arguments:
            ...
            total_plans {int}: total plans generated so far
            (deprecated) custom_ext {str}: custom additions to the filename (usually other args) 
        ...
        with open(grasps_filename, 'w') as grasps_file:
            ...
            json.dump(grasps_description, grasps_file)
        grasps_file.close()
        print('{}: {} grasps ({} plans),  for {} gripper'.format(body_name, len(body_grasps), current, gripper_name))            
